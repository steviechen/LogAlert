...
from src.main.repository.es.LogExRawForES import LogExRawForES
...
class LogExTrainUtil(object):
    ...
    def genFilterDataByTrackingIds(self,processName,returnDict,trackingIds,reqinterval='7d'):
        ...
        for trackingId in trackingIds:
            rawLogs = []
            try:
                rawLogs = LogExRawForES(trackingId.desc.dc,trackingId.desc.component).getExLogByTrackingId(trackingId.name,trackingId.desc.servertype,reqinterval)
            except Exception as e:
                print(e)
            filterLogs = self.logExProcessUtil.getImpLogsFromAll(rawLogs)
    ...
    def genFilterFileBytrackingIdsByMutiproc(self,trackingIds,reqinterval='7d',processNum = 10,rawDataTag = 'default'):
        ...
        pd.DataFrame(filterData,columns=['id','log','tag','dc']).to_csv(self.logExProcessUtil.deleteExistFile(self.rootPath+'/Logex/input/resFilter_%s.csv'%rawDataTag))
        return
    ...
    def appendFilterFileBytrackingIdsByMutiproc(self,trackingIds,reqinterval='7d',processNum = 10,rawDataTag = 'default'):
        ...
        savePath = self.rootPath+'/Logex/input/resFilter_%s.csv'%rawDataTag
    ...
    def getAllTrackingId(self,rawDataTag = 'default'):
        if (rawDataTag == 'default'): rawDataTag = self.trainTag
        resFilter = pd.read_csv(self.rootPath+'/Logex/input/resFilter_%s.csv'%rawDataTag)
    ...
    def deleteByTrackingId(self,trackingIds,rawDataTag = 'default'):
        ...
        pd.DataFrame(resFilter,columns=['id','log','tag','dc']).to_csv(self.logExProcessUtil.deleteExistFile(self.rootPath+'/Logex/input/resFilter_%s.csv'%rawDataTag))
        return
    ...
    def genFilterFileByFilePathOnMutiproc(self,component,servertype,filePath='defalutPath',reqinterval='7d',processNum =10,rawDataTag='default'):
        if (filePath == 'defalutPath'): filePath = self.rootPath + '/Logex/rawdata/trackingIds_%s.csv' % (self.trainTag)
        ...
        for i in dataline:
            if dataline.line_num == 1:
                continue
            trackingIds.append(LogExTrackingId(i[0],'',Desc(component,servertype,i[1])))
    ...
    def genSplitedData(self,rawCSVPath='defalutPath'):
        ...
        cw = lambda x: LogExProcessUtil().splitWordSentence(x)
        rawData = pd.read_csv(rawCSVPath).groupby("id").agg(self.logExProcessUtil.value_append).reset_index()
        splited = rawData['log'].map(lambda x:str(x)).apply(cw)
        return splited

    def genRawData(self,rawCSVPath='defalutPath'):
        if(rawCSVPath == 'defalutPath'):rawCSVPath = self.rootPath+'/Logex/input/resFilter_%s.csv'%self.trainTag
        rawData = pd.read_csv(rawCSVPath).groupby("id",as_index=False).agg(self.logExProcessUtil.value_append).reset_index()
        return rawData

    def genTrainDataForD2V(self, splited):
        doc_f = codecs.open(self.logExProcessUtil.deleteExistFile(self.rootPath+'/Logex/output/corpus/d2v_%s.txt'%(self.trainTag)),'w', encoding='utf8')
    ...
    def genD2VModelByDBOW(self,docListPath='defaultPath'):
        if(docListPath=='defaultPath'):docListPath=self.rootPath+'/Logex/output/corpus/d2v_%s.txt'%(self.trainTag)
        ...
        d2v.save(self.logExProcessUtil.deleteExistFile(self.rootPath+'/Logex/output/model/d2v_dbow_%s.model'%(self.trainTag)))
        return d2v
    ...
    def genW2VModel(self,documents):
        ...
        w2v.save(self.logExProcessUtil.deleteExistFile(self.rootPath+'/Logex/output/model/w2v_%s.model'%(self.trainTag)))
        return w2v
